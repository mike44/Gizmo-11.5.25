...
unsigned int xplugins_rotl(unsigned int value, int shift) {
    if ((shift &= 31) == 0)
      return value;
    return (value << shift) | (value >> (32 - shift));
}

unsigned int xplugins_rotr(unsigned int value, int shift) {
    if ((shift &= 31) == 0)
      return value;
    return (value >> shift) | (value << (32 - shift));
}
...
#define RotByte(a) xplugins_rotr(a,8)
#define RotByteL(a) xplugins_rotl(a,8)
...
int AES::EncryptBuffer( char* clean_buffer, unsigned char* key, char* ret_buffer, int size ){

		AES crypt;
		crypt.SetParameters(128);
		crypt.StartEncryption(key);
		crypt.Encrypt(reinterpret_cast<const unsigned char*>(clean_buffer),reinterpret_cast<unsigned char*>(ret_buffer),size/16);

	return 0;
}

int AES::DecryptBuffer( char* aes_buffer, int aes_buffer_size, unsigned char* key, char* ret_buffer, int original_size ){


	#if __POWERPC__

			int aes_block_count = aes_buffer_size / 16; //we need to loop this many times to decode the entire stream. boring.
				...
				int aes_block_offset=0;
				for(int aes_bx=0; aes_bx < aes_block_count; aes_bx++ ){

					memset( in_block, 0, 16 );
					memset( out_block, 0, 16 );

					memcpy( in_block, aes_buffer+aes_block_offset, 16 );

						AES_cbc_encrypt(in_block, out_block, 16, &aeskey, iv, AES_DECRYPT);

					///memcpy( in_block, iv, 16 );
					memcpy( ret_buffer+aes_block_offset, out_block, 16 );
				...
				//core debug output
				//printf("Gizmo: OpenSSL AES Decrypt yields: (%s)\n", ret_buffer);
			...
			crypt.Decrypt(reinterpret_cast<const unsigned char*>(aes_buffer),reinterpret_cast<unsigned char*>(ret_buffer),aes_buffer_size/16);
...
int AES::EncryptFile(const char * in_file, const char* out_file, const char* string_key)
	{

        ifstream ifile(in_file,ios_base::binary);
        ...
        int size = (fsize+15)&(~15); //this is used to pad out the buffers to suit the AES alg's
        ...
        char * obuffer = new char[size];
                ...
                memcpy( key, string_key, 16 );

                AES::EncryptBuffer( ibuffer, key, obuffer, size );
        ...
        char* file_buffer = new char[size + 4];
        memcpy( file_buffer, &original_size, 4 ); //first 4 bytes of the output file tell us the original size of the input data.
        memcpy( file_buffer+4, obuffer, size );
        ...
        ofstream ofile(out_file,ios_base::binary);
        ofile.write(file_buffer,size+4);
        ofile.close();
...
int AES::DecryptFile(const char * in_file, const char* out_file, const char* string_key){
        ...
        //printf("in_file size: %i\n", fsize);


        char * file_buffer = new char[fsize];
        ifile.read(file_buffer,fsize);
        ifile.close();


        //read out the first four bytes to give us the size of the original clean data
        int original_size=0;
        memcpy( &original_size, file_buffer, 4 );
        //printf("Clean payload size: %i\n", original_size);

        if( original_size > fsize ){
                printf("Data corruption. Clean-Payload size is greater than encrypted file size.\n");
                //exit(1);
                return 1;
        }


        //the AES encrypted buffer is 4 bytes smaller than the data on disk.
        int aes_buffer_size = (fsize-4);
        //printf("AES Buffer size: %i\n", aes_buffer_size);


        char* aes_buffer = new char[aes_buffer_size]; //this is the input blob
        memcpy( aes_buffer, file_buffer+4, aes_buffer_size ); //trim off the first four bytes from disk


        char* clean_buffer = new char[aes_buffer_size];	//this is the output blob
        ...
        int aes_buffer_size_check = (aes_buffer_size+15)&(~15);
        if( aes_buffer_size_check != aes_buffer_size ){
                printf("Invalid input data. Block alignment error.\n");
                //exit(1);
                return 1;
        }
                ...
                //printf("Calling AES::DecryptBuffer..\n");
                AES::DecryptBuffer( aes_buffer, aes_buffer_size, key, clean_buffer, original_size );


        //printf("Decryption finished..\n");
        delete[] aes_buffer; //wont be needing this anymore.
        ...
        char* cstring_buffer = new char[original_size+1]; //+1 for a null byte on the end.
        memset( cstring_buffer, 0, original_size+1 );

        memcpy( cstring_buffer, clean_buffer, original_size );
        ...
        //printf("Decrypted buffer content: (%s)\n", cstring_buffer);
                ...
                if( true ){
                        //printf("Writing cleaned data to:(%s)\n", out_file);
                        ofstream ofile(out_file,ios_base::binary);
                        ofile.write(clean_buffer,original_size);
                        ofile.close();
                }
...
int AES::DecryptFileToBuffer(const char* in_file, char* out_buffer, const char* string_key){
        ...
        #if __POWERPC__
                bswapi( &original_size );
        #endif


        if( original_size > fsize ){
                //out_buffer = (char*)malloc( 1024 );
                sprintf(out_buffer, "Data corruption. Clean-Payload size is greater than encrypted file size. %i %i\n", original_size, fsize);
                //exit(1);
                return 1;
        }
        ...
        int aes_buffer_size_check = (aes_buffer_size+15)&(~15);
        if( aes_buffer_size_check != aes_buffer_size ){
                sprintf(out_buffer, "AES Block Size Check: Data corruption. Clean-Payload size is greater than encrypted file size.\n");
                return 1;
        }
        ...
        //this strips off a null char at the end of string_key
        unsigned char key[16];
        memcpy( key, string_key, 16 );


                //printf("Calling AES::DecryptBuffer..\n");
                AES::DecryptBuffer( aes_buffer, aes_buffer_size, key, out_buffer, original_size );
...
int AES::PayloadSizeForFile(const char * in_file){

	//load and decode the payload size 
	//we must do this because the AES code will pad-out the original data that we fed in.
	//using this allows us to recover the original data properly.
	...
	if( fh != NULL ){
		...
		//printf("AES Get Payload Size: failed to open file: (%s)\n", in_file);
		return -1;
	}
	...
	//read out the first four bytes to give us the size of the original clean data
	int original_size=0;
	memcpy( &original_size, tmp, 4 );

	//printf("AES payload size raw: %i\n", original_size);

	#if __POWERPC__
		bswapi( &original_size );
	#endif

	//printf("AES payload size BSW: %i\n", original_size);
