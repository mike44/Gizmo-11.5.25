...
//replace some core functions if we're running on PowerPC big endian iron.
//#if APL
	#if __POWERPC__	
...
public:
	...
	static int DecryptBuffer( char* aes_buffer, int aes_buffer_size, unsigned char* key, char* ret_buffer, int original_size );
	static int EncryptBuffer( char* clean_buffer, unsigned char* key, char* ret_buffer, int size );

	static int DecryptFile(const char * in_file, const char* out_file, const char* string_key);
	static int DecryptFileToBuffer(const char * in_file, char* out_buffer, const char* string_key);
	static int EncryptFile(const char * in_file, const char* out_file, const char* string_key);


	static int PayloadSizeForFile( const char* in_file );
